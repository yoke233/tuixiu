...
- `docs/05_process/project-context.md`：棕地事实与约束（目录结构、命名/格式、测试命令、PR 规则、禁改目录、常见坑）
- `docs/05_process/definition-of-done.md`：DoD 清单（实现/测试/文档/安全/回滚/审计）
...
- 新增 `docs/05_process/project-context.md`（手工维护即可）
- 新增 `docs/05_process/definition-of-done.md`（review 与 merge 的最低标准）
- 新增 `gate_decision` Artifact schema（不一定要改 DB，先用 JSON content 约定；见 `docs/_meta/gate_decision.schema.json`）

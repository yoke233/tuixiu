# 后台配置分层 + 外部 Issue 导入 + Agent 角色模板（GitHub/GitLab）PRD

> 说明：本文是产品 PRD（需求规格），用于指导后续实现与排期。实现计划另立 `docs/plans/*-implementation.md`（如需要）。
...
- **Proposed Solution**：在后台增加“配置与凭据”能力（默认全局 + 项目覆盖），支持从 GitHub 拉取并导入/绑定外部 Issue（MVP 先 GitHub）；引入 Role Templates（角色模板）用于管理默认提示词与初始化（MVP 先 `promptTemplate + initScript`，skills/CLI 参数后续再补），并在发起 Run 时固化解析后的配置快照。
...
- **Acceptance Criteria**
  ...
  - 角色模板支持“初始化能力”，用于在 Run 启动前准备环境：
    ...
    - 系统必须保证每个 Run 使用**独立 workspace 目录**（建议 `run-<runId>`），并以该目录作为 `cwd` 执行 `initScript` 与后续 agent 任务，避免不同项目/不同 Run 互相污染。

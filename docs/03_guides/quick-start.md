...
先按 `docs/03_guides/environment-setup.md` 完成环境与数据库准备（`backend` 的 `pnpm dev` 会自动应用 Prisma 迁移），然后在三个终端分别启动：
...
- `POST /api/runs/:id/merge-pr`：高危动作，默认要求审批（返回 `APPROVAL_REQUIRED` 并创建审批请求）

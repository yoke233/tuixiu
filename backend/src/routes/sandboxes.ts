    ...
    server.post(
      ...
      async (request) => {
        const bodySchema = z.object({
          ...
          proxyId: z.string().trim().min(1).max(100).optional(),
          action: z.enum(["inspect", "ensure_running", "stop", "remove", "report_inventory"]),
        });
        ...
        if (body.action === "report_inventory") {
          if (!body.proxyId) {
            return { success: false, error: { code: "BAD_REQUEST", message: "action=report_inventory 需要 proxyId" } };
          }
          await deps.sendToAgent(body.proxyId, { type: "sandbox_control", action: "report_inventory" } as any);
        ...
        if (body.runId) {
          ...
          if (!(run as any).agent) {
            return { success: false, error: { code: "NO_AGENT", message: "该 Run 未绑定 Agent，无法下发控制命令" } };
          }
          ...
          const instanceName =
            typeof (run as any).sandboxInstanceName === "string" && (run as any).sandboxInstanceName.trim()
        ...
        if (!requestedInstanceName) {
          return { success: false, error: { code: "BAD_REQUEST", message: "需要提供 runId 或 instanceName" } };
        }

        if (body.proxyId) {
          const sandbox = await deps.prisma.sandboxInstance.findUnique({
            where: { proxyId_instanceName: { proxyId: body.proxyId, instanceName: requestedInstanceName } } as any,
          } as any);
          if (!sandbox) {
          ...
          if (!proxyId) {
            return { success: false, error: { code: "BAD_DATA", message: "SandboxInstance.proxyId 缺失" } };
          }
            ...
            await deps.prisma.event
              .create({
                data: {
                  ...
                  source: "user",
                  type: "sandbox.control.requested",
                  payload: { action: body.action, proxyId, instance_name: requestedInstanceName } as any,
                } as any,
              } as any)
              .catch(() => {});
          }
        ...
        if (resolved.length > 1) {
          return { success: false, error: { code: "CONFLICT", message: "instanceName 存在于多个 proxy，请指定 proxyId" } };
        }

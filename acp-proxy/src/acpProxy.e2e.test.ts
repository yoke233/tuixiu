...
describe("acp-proxy (e2e)", () => {
  ...
  const entrypointEnabled = process.env.ACP_PROXY_E2E_ENTRYPOINT === "1";
  ...
  it.runIf(enabled && entrypointEnabled)(
    "connects to orchestrator, registers, opens run via sandbox (entrypoint + init)",
    async () => {
      const image =
        process.env.ACP_PROXY_E2E_IMAGE?.trim() ??
        (await pickSandboxImageFromConfig());
      ...
      const cfgPath = path.join(
        tmpdir(),
        `acp-proxy-e2e-entrypoint-${Date.now()}-${Math.random()}.json`,
      );
      const workspace = path.join(
        tmpdir(),
        `acp-proxy-e2e-entrypoint-ws-${Date.now()}-${Math.random()}`,
      );
      await writeFile(
        cfgPath,
        JSON.stringify(
          {
            ...
            agent_command: [
              ...
              [
                ...
                "if (!fs.existsSync('/workspace/.init_done')) {",
                "  console.error('missing init');",
                "  process.exit(2);",
                "}",
                "process.stdout.write('{\"jsonrpc\":\"2.0\",\"method\":\"ready\"}\\n');",
                "setTimeout(()=>{}, 60000);",
      ...
      try {
        ...
        const runId = "run-e2e-entrypoint-1";
        const openedP = waitForMessageTypeOrExit({
          ws,
          proc,
          type: "acp_opened",
